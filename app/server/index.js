...
function getPluginData(pluginName, callback) {
  ...
  command.on('close', function (code) {
    console.log('getPluginData.onClose :: ' + pluginName + ' exited with putput: ' + output);
...
wsServer.on('request', function (request) {
  ...
  wsClient.on('message', function (wsReq) {
    ...
    var sendDataToClient = function (code, output) {
      if (code === 0) {
        var scriptOutput = output.join('');
        if (scriptOutput === '') {
          scriptOutput = '"{}"';
        }
        var wsResponse = '{ "moduleName": "' + moduleName + '", "output": ' + scriptOutput + ' }';
